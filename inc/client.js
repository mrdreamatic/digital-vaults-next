...
class client {
  init = async (req, window = undefined) => {
    ...
    if (window !== undefined) {
      ...
      helper.user = await getRedirectResult(fAuth)
        .then((r) => {
  ...
  request = async (req, attr = {}) => {
    ...
    const host =
      attr.host === undefined ? helper.getSiteUrl(window) + "/api" : attr.host;
    ...
    const body =
      attr.body === undefined ? JSON.stringify({}) : JSON.stringify(attr.body);
    const query =
      attr.query === undefined
        ? new URLSearchParams({}).toString()
        : new URLSearchParams(attr.query).toString();
    let response;
    switch (method) {
      case "POST":
  ...
  VerifyPasswordLess = async (user) => {
    ...
    if (
      !user &&
      isSignInWithEmailLink(this.helper.firebase.auth, window.location.href)
    ) {
    ...
    switch (using) {
      ...
      case "passwordless":
        return sendSignInLinkToEmail(
          this.helper.firebase.auth,
          attr.email,
          this.helper.firebaseAuthorize(window)
        );
      default:
  ...
  form = (x, option = [], key) => {
    ...
    switch (type) {
      case "image":
        return (
          <div className="form-group text-end" key={key}>
                    ...
                    return v.map((w, j) => {
                      return (
                        <li className="list-group-item">
                          <h6 className="m-0 mb-1 text-dark" key={j}>
                            {w.title}
                          </h6>
                          {w.option.map((z, k) => {
                            return (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  {...x.attr}
                                  id={x.name + `_${i}_${j}_${k}`}
                                  name={x.name}
                                  className="form-check-input"
                                  checked={x.dataChecked.includes(z.text)}
                                  defaultValue={z.text}
                                />{" "}
                                <label
                                  className="form-check-label"
                                  htmlFor={x.name + `_${i}_${j}_${k}`}
                                >
                                  {z.text}
                                </label>
                              </div>
              ...
              return (
                <div key={i} className="form-check form-check-inline">
                  <input
                    ...
                    defaultValue={v.val}
                  />{" "}
                  <label className="form-check-label" htmlFor={x.name + v.val}>
                    {v.text}
                  </label>
                </div>
              );
      ...
      case "select":
        ...
        return (
          <div className="form-group" key={key}>
              ...
              {option.map((v, i) => {
                return (
                  <option
                    ...
                    value={v.val}
                    selected={
                      x.attr.multiple !== undefined &&
                      x.attr.multiple &&
                      Array.isArray(x.selected)
                        ? x.selected.includes(v.val)
                        : x.selected === v.val
                    }
                  >
                    {v.text}
            ...
            {x.attr.multiple !== undefined && x.attr.multiple && (
          ...
          <>
            ...
            <div className="input-group mb-2" key={key}>
              ...
              <input
                ...
                className="form-control"
                name={`phoneCode`}
                value={country !== null ? `${country.phonecode}` : ""}
              />
            </div>
          </>
        );
